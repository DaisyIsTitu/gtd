...
### Frontend 작업
#### Todo 관리 UI
- [ ] S2.F.1 Todo 추가 모달 (제목, 소요시간, 카테고리, 우선순위, 태그)
- [ ] S2.F.2 Todo 목록 표시 및 상태별 필터링
- [ ] S2.F.3 Todo 수정/삭제 기능
- [ ] S2.F.4 상태 관리 라이브러리 설정 (Zustand/Context API)

#### 캘린더 연동 기초
- [ ] S2.F.5 Todo를 캘린더에 정적 배치 표시
- [ ] S2.F.6 색상 체계 구현 (업무/개인 카테고리별)
- [ ] S2.F.7 시간 슬롯 기본 표시 (10AM-8PM)
- [ ] S2.F.8 Mock API와 연동된 데이터 흐름
